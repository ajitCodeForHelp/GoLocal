...
function MenuItems() {
    ...
    const [items, setItems] = useState([
        ...
        },
        {
            id: 3,
            name: "Minty Melon",
            description: "Mint Leaves + Mango Crush + Mint Syrup & Mango Juice",
            price: 219,
            oldPrice: 299,
            image: img,
            quantity: 0
    ...
    return (
        <>
            <div className="menu-tab" style={{ width: "53%", backgroundColor: "white" }}>
                <div className="d-flex justify-content-between align-items-center p-3 pt-4 mb-3" style={{
                    position: "sticky",
                    top: 0,
                ...
                <div className="container ">
                    {items.map((item) => (
                        <div key={item.id} className="d-flex align-items-center p-3 border rounded mb-3 bg-white shadow-sm">
                            <img src={item.image} alt={item.name} className="rounded" style={{ width: "100px", height: "100px", objectFit: "cover" }} />

                            <div className="ms-3 flex-grow-1">
                                <h5 className="fw-bold">{item.name}</h5>
                                <p className="text-muted small">{item.description}</p>
                                <p className="fw-bold">₹ {item.price} <span className="text-decoration-line-through text-muted">₹ {item.oldPrice}</span></p>
                            </div>
