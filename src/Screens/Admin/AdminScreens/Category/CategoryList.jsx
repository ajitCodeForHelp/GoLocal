...
function Category() {
    ...
    const apiRes = async (fill) => {
        try {
            ...
            const res = await fetch(`${BASE_URL}/vendor/v1/${restaurantId}/category/list/${fill}`, {
                headers: {
    ...
    const handleStatus = async (type, id) => {
        ...
        const restaurantId = sessionStorage.getItem("restaurantId");
        try {
            const res = await fetch(`${BASE_URL}/admin/v1/${restaurantId}/${action}/${id}`, {
                method: "PUT",
                headers: {
                    'Authorization': `Bearer ${token}`,
    ...
    const handleConfirmation = (typ, i) => {
        setType(typ);
        setId(i);
        btnText(true, "cancel", "ok", `Are you sure you want to ${typ} this Category ?`);
    };
    const update = async (id, item) => {
        navigation(`/admin/category/edit/${id}`, { state: item })
    };
                                    ...
                                    return (
                                        <>
                                            <div className="products-row">
                                                ...
                                                <div className="product-cell category"><span className="cell-label">Category:</span>{item.parentCategory}</div>
                                                <div className="product-cell status-cell">
                                                    <span className="cell-label">Status:</span>
                                                    <span className={`status ${item.active ? "active" : "disabled"}`}>Active</span>
                                                </div>
