...
function Restaurants() {
    ...
    const handleStatus = async (type, id) => {
        ...
        try {
            const res = await fetch(`${BASE_URL}/admin/v1/restuarant/${action}/${id}`, {
                method: "PUT",
                headers: {
                    'Authorization': `Bearer ${token}`,
    ...
    const update = async (id) => {
        try {
            ...
            const getRes = await res.json();
            if (getRes.errorCode === 0) {
                navigation(`/admin/restaurantUpdate/${id}`, { state: getRes.responsePacket })
            }
        } catch (e) {
            console.log(e, "error in update res");
        }
    };
                                        ...
                                        <>
                                            <div className="products-row">
                                                ...
                                                <div className="product-cell action">
                                                    ...
                                                    <span className="status bg-blue" style={{ fontSize: "17px", }} onClick={()=>update(item.id)}><span style={{ marginRight: "10px" }}>Edit </span> <FiEdit /></span>
